 ...
 *
 * Simplified flow for service orders:
 * 1. 儲值: G幣入帳 users.gold_balance (已由 P99 return/notify 完成)
 * 2. 服務消費: 直接從 G幣扣除 (不經過遊戲點數)
 *
 * Net result: G幣 +X (deposit) -X (service) = 0
 */
function fulfillServiceOrder(p99OrderId, callback) {
    ...
    db.get(
        ...
        (err, serviceOrder) => {
            ...
            console.log(`[P99Pay ServiceFulfill] Processing ${serviceOrder.order_id}: ${serviceOrder.gold_amount}G for service ${gameId}`);
            ...
            db.serialize(() => {
                ...
                db.run(
                    ...
                    function(err) {
                        ...
                        // Log service consumption transaction
                        db.run(
                        ...
                        // Mark service order as fulfilled
                        db.run(
                        ...
                        db.run('COMMIT', (err) => {
                            ...
                            console.log(`  - 服務消費: -${serviceOrder.gold_amount}G`);
