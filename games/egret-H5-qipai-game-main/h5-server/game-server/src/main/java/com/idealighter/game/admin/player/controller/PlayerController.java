...
public class PlayerController {
  ...
  // ============ GameZoe Platform Integration APIs ============

  /**
   * 根據平台用戶 code 獲取金幣餘額.
   *
   * @param code 平台用戶 code (如 GZ_102746929077306565219).
   * @return 包含 gold 和 playerId 的結果.
   */
  @Path("/getGoldByCode")
  @GET
  public Result getGoldByCode(@NotEmpty @QueryParam("code") String code) {
    ...
    if (playerBo != null) {
      ...
      result.getMap().put("safeGold", playerBo.getSafeGold().get());
      result.getMap().put("playerName", playerBo.getPlayerName());
    }
    return result;
  }
   ...
   * 根據平台用戶 code 增加金幣.
   *
   * @param code 平台用戶 code.
   * @param gold 增加的金幣數量 (正數).
   * @return 結果.
   */
  @Path("/addGoldByCode")
  @POST
  public Result addGoldByCode(
      @NotEmpty @FormParam("code") String code,
      @NotNull @Min(1) @FormParam("gold") Long gold) {
   ...
   *
   * @param code 平台用戶 code.
   * @param gold 扣除的金幣數量 (正數，會轉為負數).
   * @return 結果.
   */
  @Path("/deductGoldByCode")
  @POST
  public Result deductGoldByCode(
      @NotEmpty @FormParam("code") String code,
      @NotNull @Min(1) @FormParam("gold") Long gold) {
    ...
    if (playerBo != null) {
      // 檢查餘額是否足夠
      long currentGold = playerBo.getGold().get();
      if (currentGold < gold) {
        result.changeErrorCode(ErrorCode.PLAYER_GOLD_NOT_ENOUGH);
        result.getMap().put("currentGold", currentGold);
        result.getMap().put("requestedDeduction", gold);
        return result;
      }
      ...
      if (player != null) {
        // 在線玩家
        playerMgr.addGold(player, -gold, LogReason.BACKEND_MINUS_GOLD);
        playerMgr.updatePlayer(player.playerBo());
      } else {
        // 離線玩家
        playerMgr.addGold(playerBo, -gold, LogReason.BACKEND_MINUS_GOLD);
        playerMgr.updatePlayer(playerBo);
      }
      ...
      result.getMap().put("playerId", playerBo.getId());
      result.getMap().put("gold", playerBo.getGold().get());
      result.getMap().put("message", "Successfully deducted " + gold + " gold");
