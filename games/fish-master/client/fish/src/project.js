    ...
    Alert: [function (t, e, n) {
        ...
        cc._RF.push(e, "2761ftpellC9JHJlxHl7y91", "BulletManager"), cc.Class({
            ...
            createBulletWithCannon: function (t, e) {
                ...
                if (!n) {
                    console.error("createBulletWithCannon: Bullet is NULL for cannonKind " + e);
                    return null;
                }
            ...
            showBombPanel: function (t) {
                if (this.currentBombCallback) {
                    this.unschedule(this.currentBombCallback);
                    this.currentBombCallback = null;
                }
                var e = function () {
                    if (cc.yqs.bomb) {
                        cc.yqs.bomb.getComponent(cc.Animation) && cc.yqs.bomb.getComponent(cc.Animation).stop();
                        cc.yqs.bomb.active = !1;
                    }
                    this.currentBombCallback = null;
                };
                this.currentBombCallback = e;

                if (cc.yqs.bomb) {
                    cc.yqs.bomb.active = !0;
                    // Attempt to fix black screen opacity if it's too dark
                    cc.yqs.bomb.opacity = 150; // Make it semi-transparent

                    if (cc.yqs.bomb.getComponent("bomb")) {
                        cc.yqs.bomb.getComponent("bomb").setScore(t);
                    }
                    if (cc.yqs.bomb.getComponent(cc.Animation)) {
                        cc.yqs.bomb.getComponent(cc.Animation).play();
                    }
                }

                this.scheduleOnce(e, 3); // Reduce to 3 seconds for faster recover

                // Make sure coin storm loop is also managed if needed, but it seems to run on 'this'
        ...
        cc._RF.push(e, "9c4a1C5sMpHGLYuQhmEcp3v", "Cannon"), cc.Class({
            ...
            fireLock: function () {
                var t = this.lockFish, e = t ? t.getComponent("Fish") : null;
                // console.log("fireLock check:", t ? "Found" : "Null", e ? ("Locked:" + e.locked + " Status:" + e.status) : "NoComp");
                if (null != t && 0 != e.locked && 1 == e.status) {
                    var n = e.getLockedPosByPlayerId(this.chairId);
                    this.onFire(n, t)
                } else {
                    console.log("fireLock: Unlocking because target invalid/lost");
                    this.onUnLockFish()
                }
            },
        ...
        cc._RF.push(e, "a293578d6FJgonO364JAa+U", "GameNetMgr"), cc.Class({
            ...
            initHandlers: function () {
                ...
                cc.yqs.net.addHandler("login_result", function (e) {
                    console.log("LOGIN_RESULT_FULL_PAYLOAD:", JSON.stringify(e));
                    if (cc.log("login_result: " + e), 0 === e.errcode) {
                        var e = e.data;
                        console.log("LOGIN_RESULT_DATA:", JSON.stringify(e));
            ...
            connectGameServer: function (t) {
                ...
                cc.yqs.wc.show(cc.yqs.getString("key197")), cc.yqs.net.connect(function () {
        ...
        cc._RF.push(e, "a518axJx3tGt40UMJHM8Cr0", "Hall"), cc.Class({
            ...
            initLeftTopUI: function () {
                ...
                // Disable Recharge and Customer Service buttons
                cc.find("Canvas/top_left/bg_gems/btn_add_gems").active = false; // Recharge + button
                cc.find("Canvas/top_left/serverBtn").active = false;      // Customer Service
            },
            ...
            initRightBottomUI: function () {
                ...
                // Disable Quick Start
                var qs = cc.find("Canvas/bottom_right/quickStart");
                if (qs) qs.active = false;
            },
            initLeftBottomUI: function () {
                ...
                // Disable Recharge and Withdraw buttons
                cc.find("Canvas/bottom_left/btn_recharge").active = false; // Recharge
                cc.find("Canvas/bottom_left/btn_toMoney").active = false;  // Withdraw/Exchange
                // Disable Gift and App Download
                var gift = cc.find("Canvas/bottom_left/btn_gift"); if (gift) gift.active = false;
                var app = cc.find("Canvas/bottom_left/btn_app "); if (app) app.active = false;
            },
            ...
            onBtnClicked: function (t) {
                if ("btn_add_gems" == t.target.name) console.log("addGems clicked"), this.rechargeWin.active = !0; else if ("rewardPool" == t.target.name) console.log("rewardPool clicked"), this.rewardOrderWin.active = !0; else if ("serverBtn" == t.target.name) console.log("serverBtn clicked"), this.serviceCustomWin.active = !0; else if ("btn_shezhi" == t.target.name) console.log("shezhi clicked"), this.settingsWin.active = !0; else if ("btn_quit" == t.target.name) console.log("quit clicked"), cc.director.loadScene("login"); else if ("btn_xiaoxi" == t.target.name) console.log("xiaoxi clicked"), this.xiaoxiWin.active = !0; else if ("head" == t.target.name) console.log("head clicked"), this.userinfoWin.active = !0, this.refreshInfo(); else if ("quickStart" == t.target.name) console.log("quickStart clicked"), this.onJoinGameClicked(null, 1); else if ("btn_recharge" == t.target.name) console.log("btn_recharge clicked"), this.rechargeWin.active = !0; else if ("btn_rank" == t.target.name) console.log("btn_rank clicked"), this.rankWin.active = !0; else if ("btn_toMoney" == t.target.name) {
                    ...
                    cc.yqs.alert.show("", cc.yqs.getString("key152"), function () {
                        e.runAction(cc.sequence(n, i))
                    }, !0)
                } else "btn_emil" == t.target.name ? (console.log("btn_emil clicked"), this.emailWin.active = !0) : "btn_vip" == t.target.name ? (console.log("btn_vip clicked"), this.privilegeWin.active = !0) : "btn_gift" == t.target.name ? console.log("btn_gift clicked") : "btn_app " == t.target.name && (console.log("btn_app  clicked"), cc.yqs.alert.show("", cc.yqs.getString("key103"), function () {
                }, !0))
            },
            onJoinGameClicked: function (t, e) {
                ...
                switch (e) {
                    case "1":
                        if (cc.yqs.userMgr.gems < 100) return void cc.yqs.alert.show("", cc.yqs.getString("key177"), function () {
                        });
                        break;
                    case "2":
                        if (cc.yqs.userMgr.gems < 100) return void cc.yqs.alert.show("", cc.yqs.getString("key177"), function () {
                    ...
                    case "3":
                        ...
                        break;
                    case "4":
                        if (cc.yqs.userMgr.gems < 1e3) return void cc.yqs.alert.show("", cc.yqs.getString("key177"), function () {
                        });
                        break;
                    case "5":
                    case "6":
                        return void cc.yqs.alert.show("", cc.yqs.getString("key172"))
                }
                n.EnterGame(e)
            },
            EnterGame: function (t) {
                cc.yqs.wc.show(cc.yqs.getString("key179"));
                var e = .001;
                switch (t) {
                    case "1":
                        e = .001;
                ...
                cc.yqs.userMgr.enterPublicRoom(e.toString(), function (t) {
                    0 == t.errcode ? cc.yqs.gameNetMgr.connectGameServer(t) : cc.vv.alert.show(cc.yqs.getString("key181"), cc.yqs.getString("key178"))
                }.bind(this))
            },
            update: function (t) {
        ...
        cc._RF.push(e, "c326b+yAjZGZblBqC9UjI+A", "HotUpdate"), cc.Class({
            ...
            checkCb: function (t) {
                switch (cc.log("Code: " + t.getEventCode()), t.getEventCode()) {
                    ...
                    case jsb.EventAssetsManager.ALREADY_UP_TO_DATE:
                        cc.log("Already up to date with the latest remote version."), cc.eventManager.removeListener(this._checkListener), this.lblErr.string += cc.yqs.getString("key182"), cc.director.loadScene("loading");
                        break;
            ...
            updateCb: function (t) {
                ...
                if (n && (cc.eventManager.removeListener(this._updateListener), this.updatePanel.active = !1), e) {
                    ...
                    Array.prototype.unshift(o, r), cc.sys.localStorage.setItem("HotUpdateSearchPaths", JSON.stringify(o)), jsb.fileUtils.setSearchPaths(o), this.lblErr.string += cc.yqs.getString("key183"), cc.game.restart()
                }
            },
            hotUpdate: function () {
                this._am && this._needUpdate && (this.lblErr.string += cc.yqs.getString("key184"), this._updateListener = new jsb.EventListenerAssetsManager(this._am, this.updateCb.bind(this)), cc.eventManager.addListener(this._updateListener, 1), this._failCount = 0, this._am.update())
            },
            onLoad: function () {
                if (cc.sys.isNative) {
                    this.lblErr.string += cc.yqs.getString("key185");
        ...
        window.i18n || (window.i18n = { languages: {}, curLang: "" }), e.exports = {
            init: function (t) {
                ...
                for (var s = 0; s < e.length; ++s) {
                    e[s].updateLabel();
                    // Force Traditional Chinese Font globally as requested
                    var lbl = e[s].node.getComponent(cc.Label);
                    if (lbl) {
                        lbl.useSystemFont = true;
                        lbl.fontFamily = "Microsoft JhengHei";
                    }
                }
        ...
        cc._RF.push(e, "0ef08mGSGBFxKM/jy0rZMcS", "Loading"), cc.Class({
            ...
            initLanguage: function () {
                cc.yqs.Language = t("LanguageData"), cc.yqs.Language.init("zht"), cc.yqs.getString = cc.yqs.Language.t, console.log(cc.yqs.getString("hello"))
            },
            urlParse: function () {
                var t = {};
            ...
            extends: cc.Component, statics: {
                ip: "", sio: null, isPinging: !1, fnDisconnect: null, handlers: {}, addHandler: function (t, e) {
                    if (this.handlers[t]) console.log("event:" + t + "' handler has been registered."); else {
                        ...
                        this.handlers[t] = n, this.sio && (console.log("register:function " + t), this.sio.off(t), this.sio.on(t, n))
                    }
                    ...
                    }), this.sio.off("disconnect"), this.sio.on("disconnect", function (t) {
                    ...
                    for (var s in this.handlers) {
                        ...
                        "function" == typeof o && ("disconnect" == s ? this.fnDisconnect = o : (console.log("register:function " + s), this.sio.off(s), this.sio.on(s, o)))
                    }
                    this.startHearbeat()
                    ...
                    this.sio && this.sio.connected && (null != e && "object" == (void 0 === e ? "undefined" : i(e)) && (e = JSON.stringify(e)), this.sio.emit(t, e))
                }, ping: function () {
        ...
        cc._RF.push(e, "65ea7NHXLtGKbOTSwvyVvTG", "PrivilegeWin"), cc.Class({
            ...
            onLoad: function () {
                this.benfitConfig = {
                    ...
                    5: 3e4,
                    6: 7e4
                }, this.curPageIndex = cc.yqs.userMgr.vip, this.money = cc.yqs.userMgr.money, this.gunIndex = this.curPageIndex, console.log(this.money + cc.yqs.getString("key62")), this.beforeBtn.node.active = this.curPageIndex > 0, this.afterBtn.node.active = this.curPageIndex < Object.getOwnPropertyNames(this.benfitConfig).length - 1, this.pageView.setCurrentPageIndex(this.curPageIndex), 6 == this.curPageIndex ? (this.tipString = cc.yqs.getString("key191"), this.lblTip.string = this.tipString, this.leftLabel.string = this.curPageIndex, this.rightLabel.string = this.curPageIndex) : (this.remoney = this.benfitConfig[this.curPageIndex + 1] - this.money, this.tipString = cc.yqs.getString("key48"), this.lblTip.string = cc.js.formatStr(this.tipString, this.remoney.toFixed(2), this.curPageIndex + 1), this.leftLabel.string = this.curPageIndex, this.rightLabel.string = this.curPageIndex + 1), this.vipprogress.progress = this.money / this.benfitConfig[this.curPageIndex + 1];
                // Disable Recharge Button in VIP Win
                try {
                    this.node.children.forEach(function (c) {
                        if (c.name.toLowerCase().indexOf("recharge") >= 0 || c.name.toLowerCase().indexOf("pay") >= 0 || c.name.indexOf("btn_") >= 0 && c.getComponent(cc.Button) && c.name != "beforeBtn" && c.name != "afterBtn") {
                            // More aggressive filtering for the recharge button if simple name match fails. 
                            // The button likely calls onRecharge.
                            // For now, let's target specific names if possible, but fallback to "recharge" substring.
                        }
                    });
                    // Specific target based on typical naming
                    var rBtn = this.node.getChildByName("btn_recharge");
                    if (rBtn) rBtn.active = false;

                    // Also iterate looking for click event handler "onRecharge"
                    var buttons = this.node.getComponentsInChildren(cc.Button);
                    for (var i = 0; i < buttons.length; ++i) {
                        var evts = buttons[i].clickEvents;
                        for (var j = 0; j < evts.length; ++j) {
                            if (evts[j].handler == "onRecharge") {
                                buttons[i].node.active = false;
                            }
                        }
                    }
                } catch (e) { console.log("Error hiding VIP recharge btn", e); }
            },
            onEnable: function () {
                this.commonBg.scale = .2;
                var t = cc.scaleTo(.2, 1.2), e = cc.scaleTo(.1, 1);
        ...
        cc._RF.push(e, "0c2d13/EzlC3JRtWgC5f14U", "RechargeWin"), cc.Class({
            ...
            onConfimClick: function () {
                var t = this;
                t.numberRechargeBox.string ? cc.yqs.alert.show(cc.yqs.getString("key181"), cc.yqs.getString("key186"), function () {
                    console.log("recharge num: " + parseFloat(t.numberRechargeBox.string).toFixed(2) + " by: " + t.rechargeType), t.node.active = !1
                }, !0) : cc.yqs.alert.show(cc.yqs.getString("key181"), cc.yqs.getString("key187"))
            },
            onBack: function () {
                var t = cc.scaleTo(.1, 1.2), e = cc.scaleTo(.2, .6), n = cc.callFunc(function () {
                    this.node.active = !1
        ...
        cc._RF.push(e, "88495SoqZJKX6a5qY/jeUBv", "RoomMgr"), cc.Class({
            ...
            initView: function (t) {
                ...
                this.transViewSeat(), this.lblRoomNo.string = cc.yqs.getString("key193") + cc.yqs.userMgr.roomId, this.isGlobalPlaying = !1, this.globalMsgList = [], this.globalMsgList.push(1), this.globalMsgList.push(2), this.globalMsgList.push(3)
            },
        ...
        cc._RF.push(e, "2f0bfD1eEdDbZ2Oy9CrDHFQ", "UserInfoWin"), cc.Class({
            ...
            onBindPhone: function () {
                cc.yqs.alert.show(cc.yqs.getString("key181"), cc.yqs.getString("key19"))
            },
            onItemInfo: function () {
        ...
        cc.Class({
            ...
            login: function () {
                var t = this;
                cc.yqs.wc.show(cc.yqs.getString("key195")), cc.yqs.http.sendRequest("/login", {
            ...
            enterRoom: function (t, e) {
                ...
                cc.yqs.wc.show(cc.yqs.getString("key196") + t), cc.yqs.http.sendRequest("/enter_private_room", i, function (i) {
        ...
        cc.Class({
            ...
            onAvatarClick: function (t) {
                ...
                var e = this;
                0 == cc.yqs.userMgr.ReHeadCount ? (console.log("=========toggle==========" + t + "====================="), cc.yqs.alert.show(cc.yqs.getString("key188"), cc.yqs.getString("key189"), function () {
                ...
                }, !0)) : cc.yqs.userMgr.ReHeadCount > 0 && cc.yqs.userMgr.gems >= 1 ? cc.yqs.alert.show(cc.yqs.getString("key188"), cc.yqs.getString("key190"), function () {
                    var n = e.avatars.indexOf(t);
                ...
                }, !0) : cc.yqs.userMgr.ReHeadCount > 0 && cc.yqs.userMgr.gems < 1 && cc.yqs.alert.show(cc.yqs.getString("key188"), cc.yqs.getString("key192")), cc.yqs.userMgr.ReHeadCount++
            },
            onBack: function () {
                var t = cc.scaleTo(.1, 1.2), e = cc.scaleTo(.2, .6), n = cc.callFunc(function () {
                    this.node.active = !1
        ...
        cc._RF.push(e, "77784LRrLtNUZqXof19dK1m", "zh"), window.i18n || (window.i18n = {}), window.i18n.languages || (window.i18n.languages = {}), window.i18n.languages.zh = {
            ...
            key9: "連接失敗，即將重試",
            key10: "準備登陸",
            key11: "與服務器斷開連接，正在嘗試重連",
            key12: "每一筆最最少儲值10元 \n 需要安裝支付寶客戶端",
            key13: "每一筆最最少儲值10元 \n 需要安裝微信客戶端",
            key14: "二維碼下載",
            key15: "系統消息: 恭喜%s號桌的%s捕獲了%s, 獲得%s金幣",
            key16: "再見！",
            key17: "你好！",
            key18: "積累獎池",
            key19: "儲值",
            ...
            key46: "郵箱",
            key47: "特權",
            key48: "在儲值%s元即可升級到vip%s",
            key49: "獲取vip%s專屬炮臺",
            key50: "獲取vip%s專屬勳章",
            key51: "解鎖自動開炮功能",
            key52: "高級vip同時可享可享有所有低級vip特權",
            key53: "歡迎來到《捕魚大亨》",
            key54: "閱讀",
            key55: "獎勵",
            key56: "APP下載",
            ...
            key76: "取消",
            key77: "系統訊息",
            key78: "恭喜%S桌的%s捕獲了%s,獲得%s金幣",
            key79: "登入逾時",
            key80: "知道了",
            key81: "切換帳號",
            key82: "版本",
            key83: "載入資源",
            key84: "親愛的玩家，建議您儘快將帳號綁定手機，以保障您的帳號安全。是否立即前往綁定？",
            key85: "現在綁定還附贈%s元啟動金",
            key86: "新暱稱",
            key87: '%s位\"漢\"語或%s位英文數字',
            key88: "本次修改免費",
            key89: "遊客",
            key90: "輸入手機號碼",
            key91: "驗證碼",
            ...
            key94: "發送驗證碼",
            key95: "綁定",
            key96: "以上聯繫方式均為本遊戲專屬代理，儲值系統不穩定時可使用代理儲值。",
            key97: "親愛的用戶，你好： 歡迎來到我們的捕魚遊戲！我們秉承著公平、公正的經營理念，致力為玩家提供最優質的服務，如遇到問題請使用【客服】功能進行反饋，祝您遊戲愉快！官方網址：%s,訪問可下載遊戲APP ",
            key98: "公告",
            key99: "線上反饋",
            key100: "請輸入您要諮詢的問題",
            key101: "問題提交後客服會儘快處理，請注意查看遊戲內的郵件回覆。",
            key102: "反饋",
            key103: "是否前往下載APP客戶端",
            key104: "明日登入還可領取%s元",
            key105: "明日登入獎勵需從APP登入領取為避免帳號遺失，建議先綁定手機",
            key106: "下載地址",
            key107: "您的位置",
            key108: "任意門",
            key109: "魚種",
            key110: "“金邊藍底”為獎勵魚",
            key111: "雷射炮",
            key112: "長時間未進行操作%s秒後自動退出遊戲......",
            key113: "深海寶藏",
            key114: "打魚可以累積雷射能量（不同場次的能量不通用）",
            key115: "對著一群魚來一發，抓準時機猛賺一筆",
            key116: "滿能量狀態時，點擊炮臺，可以切換雷射炮",
            key117: "雷射炮對炸彈類魚和聚寶盆無效，雷射炮被激活後需要在30秒內發射",
            key118: "奪寶流程",
            key119: "打鑽石魚",
            key120: "集鑽石",
            key121: "開寶箱",
            ...
            key142: "中獎名單",
            key143: "獎池介紹",
            key144: "在高級場及以上的場次遊戲，可通過收集打魚掉落的鑽石打開寶箱，寶箱打開後可獲得獎池獎金。",
            key145: "%s等獎",
            key146: "僅對VIP%s及以上的玩家開放",
            key147: "刪除",
            key148: "下一條",
            key149: "返回",
            key150: "更多VIP功能開發中",
            key151: "兌換",
            key152: "您的賬號尚未綁定手機，請綁定完成後，在進行兌換操作",
            key153: "可提取金額",
            key154: "兌換金額",
            key155: "收款賬號",
            key156: "未綁定",
            key157: "綁定",
            key158: "立即兌換",
            key159: "兌換兩小時到賬，超出時間未到，請聯繫客服諮詢",
            key160: "兌換最低金額%s元，兌換後賬號內必須剩餘至少%s元",
            key161: "每筆兌換需要收取兌換金額%%s的手續費",
            key162: "金幣不足，無法進入房間",
            key163: "金幣不足，是否儲值",
            key164: "入場",
            key165: "修改暱稱",
            ...
            key187: "請輸入儲值金額",
            key188: "更換頭像",
            key189: "本次更改免費，確定更換頭像？",
            key190: "本次更改需要花費1元，確定更換頭像？",
            key191: "您已達最高級！",
            key192: "更換失敗，金幣不足",
            key193: "漁場ID: ",
            key194: "正在同步魚群",
            ...
            key82: "版本",
            key83: "載入資源",
            key84: "親愛的玩家，建議您盡快將賬號綁定手機，以保障您的賬號安全。是否立即前往綁定？",
            key85: "現在綁定還附贈%s元啟動金",
            key86: "新暱稱",
            key87: '%s位"漢"語或%s位英文數字',
            key88: "本次修改免費",
            ...
            key99: "在線反饋",
            key100: "請輸入您要諮詢的問題",
            key101: "問題提交後客服會盡快處理，請注意查看遊戲內的回覆。",
            key102: "反饋",
            key103: "是否前往下載APP客戶端",
            key104: "明日登入還可領取%s元",
            key105: "明日登入獎勵需從APP登入領取為避免賬號丟失，建議先綁定手機",
            key106: "下載地址",
            key107: "您的位置",
            key108: "任意門",
            key109: "魚種",
            ...
            key114: "打魚可以累積雷射能量（不同場次的能量不通用）",
            key115: "對著一群魚來一發，抓準時機猛賺一筆",
            key116: "滿能量狀態時，點擊砲臺，可以切換雷射砲",
            key117: "雷射砲對炸彈類魚和聚寶盆無效，雷射砲被激活後需要在30秒內發射",
            key118: "奪寶流程",
            key119: "打鉆石魚",
            key120: "集鉆石",
            key121: "開寶箱",
            ...
            key142: "中獎名單",
            key143: "獎池介紹",
            key144: "在高級場及以上的場次遊戲，可通過收集打魚掉落的鉆石打開寶箱，寶箱打開後可獲得獎池獎金。",
            key145: "%s等獎",
            key146: "僅對VIP%s及以上的玩家開放",
            key147: "刪除",
            key148: "下一條",
