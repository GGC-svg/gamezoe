Game.Prefabs.GameoverPanel = function (game, parent) {
	...
	// Pause text
	// Pause text
	this.textPause = this.game.add.text(game.width / 2, -50, '遊戲結束', { font: '28px Arial', fill: '#ffffff' });
	this.textPause.anchor.setTo(0.5, 0);
	this.add(this.textPause);

	// Score text
	// Score text
	this.textScore = this.game.add.text(game.width / 2, 100, '分數 : 0', { font: '16px Arial', fill: '#ffffff' });
	this.textScore.anchor.setTo(0.5, 0);
	this.add(this.textScore);

	// Highscore text
	// Highscore text
	this.textHighScore = this.game.add.text(game.width / 2, 125, '最高分 : 0', { font: '16px Arial', fill: '#ffffff' });
	this.textHighScore.anchor.setTo(0.5, 0);
...
Game.Prefabs.GameoverPanel.prototype.show = function (score) {
	...
	this.textScore.setText('分數 : ' + score.toString());
	...
	this.textHighScore.setText('最高分 : ' + highScore.toString());
